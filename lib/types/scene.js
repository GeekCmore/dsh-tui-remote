...
function createRemoteSceneComponent(store, workspaces, config) {
    return function RemoteScene(props) {
        ...
        return React.createElement(Box, { flexDirection: 'column', width: '100%', paddingX: 1 }, React.createElement(Box, { flexDirection: 'row', justifyContent: 'space-between' }, React.createElement(Text, { bold: true }, `dsh remote  LIVE  ${config.title}`), React.createElement(Text, { color: statusColor(snapshot.status), bold: true }, snapshot.busy ?? snapshot.status)), tabRow, actionRow, snapshot.error === undefined ? null : React.createElement(Text, { color: 'error', wrap: 'wrap' }, snapshot.error), body);
    };
}
