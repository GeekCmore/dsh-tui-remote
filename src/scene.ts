...
import type { ConnectionStore, RemoteAction } from './store.js'
import type { RemoteWorkspaceController } from './workspaces.js'

const TABS = ['Overview', 'Diagnostics', 'Workspaces'] as const
const ACTIONS: readonly { label: string; action: RemoteAction }[] = [
...
function statusColor(status: string): 'success' | 'error' | 'warning' | 'subtle' {
...
function createRemoteSceneComponent(
  store: ConnectionStore,
  workspaces: RemoteWorkspaceController,
  config: ResolvedConfig,
): TuiSceneDescriptor['component'] {
  return function RemoteScene(props: TuiSceneProps) {
    ...
    useInput((input, key, event) => {
      ...
      if (key.return) {
        ...
        if (path !== undefined) openWorkspace(path)
        return
      }
      if (!key.ctrl && !key.meta && input.length > 0) {
        const text = event.isPasted ? input.replace(/[\r\n]+/g, '') : input
    ...
    const tabRow = React.createElement(
      ...
      ...TABS.map((label, index) => React.createElement(
        Text,
        { key: label, bold: tab === index, inverse: tab === index },
        ` ${label} `,
      )),
    )
    const actionRow = React.createElement(
      ...
      ...ACTIONS.map((entry, index) => React.createElement(
        Text,
        {
          key: entry.action,
    ...
    if (tab === 0) {
      ...
      const identity = React.createElement(
        Box,
        { flexDirection: 'column', width: narrow ? '100%' : '50%' },
      ...
      const health = React.createElement(
        Box,
        { flexDirection: 'column', width: narrow ? '100%' : '50%', marginTop: narrow ? 1 : 0 },
        React.createElement(Text, { bold: true }, 'Host'),
        React.createElement(Text, null, `CPU      ${metrics?.cpuBusyRatio === undefined ? '-' : `${(metrics.cpuBusyRatio * 100).toFixed(0)}%`}`),
        React.createElement(Text, null, `Memory   ${formatBytes(memoryUsed)} / ${formatBytes(metrics?.memTotalBytes)}`),
        React.createElement(Text, null, `Disk     ${formatBytes(diskUsed)} / ${formatBytes(metrics?.diskTotalBytes)}`),
        React.createElement(Text, null, `Load     ${metrics?.loadavg.map(value => value.toFixed(2)).join('  ') ?? '-'}`),
        React.createElement(Text, null, `Processes ${metrics?.processCount ?? '-'}`),
      )
      ...
      body = React.createElement(
        ...
        React.createElement(Text, { bold: true }, 'Runtime checks'),
        React.createElement(Text, { color: snapshot.status === 'connected' ? 'success' : 'warning' }, `Connection  ${snapshot.status}`),
        React.createElement(Text, { color: store.runtime.runtimeRoot?.startsWith('/') ? 'success' : 'warning' }, `Runtime root ${store.runtime.runtimeRoot ?? 'unavailable'}`),
        ...snapshot.diagnostics.map(check => React.createElement(
          Text,
          { key: check.name, color: check.status === 'ok' ? 'success' : check.status === 'missing' || check.status === 'error' ? 'error' : 'subtle' },
          `${check.name.padEnd(12)} ${check.status}${check.detail ? ` - ${check.detail}` : ''}`,
        )),
      )
    } else {
      body = React.createElement(
        Box,
        { flexDirection: 'column', marginTop: 1 },
        React.createElement(Text, { bold: true }, 'Remote directories'),
        ...(paths.length === 0
          ? [React.createElement(Text, { key: 'empty', color: 'subtle' }, 'No configured directories')]
          : paths.map((path, index) => React.createElement(
              Text,
              { key: path, inverse: draft.length === 0 && workspaceIndex === index, wrap: 'truncate-end' },
              `${draft.length === 0 && workspaceIndex === index ? '>' : ' '} ${path}`,
            ))),
        React.createElement(Box, { flexDirection: 'row', marginTop: 1 },
          React.createElement(Text, { color: 'subtle' }, 'Path  '),
          React.createElement(Text, { color: draft.length > 0 ? 'text' : 'subtle', wrap: 'truncate-end' }, draft || '/remote/absolute/path'),
        ),
        workspaceError === undefined ? null : React.createElement(Text, { color: 'error', wrap: 'wrap' }, workspaceError),
      )
    }
    ...
    return React.createElement(
      ...
      React.createElement(Box, { flexDirection: 'row', justifyContent: 'space-between' },
        React.createElement(Text, { bold: true }, `dsh remote  LIVE  ${config.title}`),
        React.createElement(Text, { color: statusColor(snapshot.status), bold: true }, snapshot.busy ?? snapshot.status),
      ),
      tabRow,
      actionRow,
      hostKeyPrompt,
